...
C/S架构之所以能够流行的一个主要原因就是网速的提高，特别是无线网络速度的提高。试想在2G时代，打开个图片都得等半天，更别说是看视频了！

网速的提高，使得越来越多的人使用网络，例如：优酷，微信都是上亿用户量，更别说天猫双11的瞬间访问量了！那服务器如何能快速的处理用户的请求呢？这就要求我们的服务器性能必须很高！
...
本文不会讨论这么多，只会从IO模型层面进行讨论!

## Socket

无论你是发邮件，浏览网页，还是看视频～实际底层都是使用的TCP/IP，而TCP/IP的编程抽象就是Socket!

我一直对Socket的中文翻译很困惑，我个人觉得是我所接触的技术名词翻译里最莫名其妙的，没有之一！
...
虽然上面说道NIO一个线程就可以支持所有的IO处理。但是瓶颈也是显而易见的！我们看一个客户端的情况，如果这个客户端多次进行请求，如果在Handler中的处理速度较慢，那么后续的客户端请求都会被积压，导致响应变慢！所以引入了Reactor多线程模型!
...
Netty开发一个很明显的问题就是回调，一是打破了线性编码习惯，
二就是Callback Hell！
...
1,2,3处代码如果是同步的，那么将按顺序执行！但是如果不是同步的呢？我还是喜欢2在1之后执行，3在2之后执行！怎么办呢？想想AJAX!我们需要写类似如下这样的代码！
